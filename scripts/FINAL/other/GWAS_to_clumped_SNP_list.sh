...
if [ "$suffix2" == "var" ]; then
  ...
  clump_snp_field=SNP
  clump_field=P
elif [ "$suffix2" == "DET" ]; then
  outdir=/athena/elementolab/scratch/anm2868/vQTL/ukb_vqtl/output/dGWAS_clump
  mkdir -p $outdir
  results=/athena/elementolab/scratch/anm2868/vQTL/ukb_vqtl/output/GWAS/HLMM_results/ukbb.${phenoName}.HLMM.dispersion_nochr6.txt
  clump_snp_field=SNP
  clump_field=dispersion_pval_10neg
fi
...
plink --bfile $geno --maf 0.05 --clump $results --clump-p1 $thres --clump-p2 $thres --clump-r2 $r2thres --clump-field $clump_field --clump-snp-field $clump_snp_field --clump-kb $kbthres --out $outdir/$outFile
done
