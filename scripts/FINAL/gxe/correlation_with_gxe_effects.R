...
# results.mg.sub <- subset(results.mg,FDR < 0.1)
# # results.mg.sub <- subset(results.mg,results.mg[,4] < 0.001 & E=='Alcohol_intake_frequency')
# results.mg.sub <- subset(results.mg,E=='Alcohol_intake_frequency')
# results.mg.sub <- subset(results.mg,E=='PA')
# 
# # results.mg.sub <- subset(results.mg,Raw.vQTL == 1 & FDR < 0.1)
# # results.mg.sub <- subset(results.mg,dQTL == 1)
# # results.mg.sub <- subset(results.mg,Rint.vQTL == 1)
# # results.mg.sub <- subset(results.mg,Rint.vQTL == 1 & Raw.vQTL==0)
# # results.mg.sub <- subset(results.mg,Mean.QTL == 1 & Raw.vQTL == 0)
# # results.mg.sub <- subset(results.mg,Mean.QTL == 1)
# 
# # cor.test(results.mg.sub$Estimate.x,results.mg.sub$P.VAR.RAW)
# 
# # cor.test(results.mg.sub$Estimate.x,results.mg.sub$Estimate.y)
# 
# 
# 
# 
# cor.test(results.mg$Estimate.x,results.mg$Estimate.y)
# cor.test(results.mg$Estimate.x,results.mg$BETA.VAR.RAW)
# 
# 
...
# matched_qtl$FDR <- p.adjust(matched_qtl[,4],method = 'fdr')
# matched_qtl.sub <- subset(matched_qtl,matched_qtl[,4] < 0.001)
# 
# cor.test(matched_qtl$Estimate.x,matched_qtl$Estimate.y)
# cor.test(matched_qtl.sub$Estimate.x,matched_qtl.sub$Estimate.y)
# 
# 
# 
